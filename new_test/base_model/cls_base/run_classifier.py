...
BERT_BASE_DIR=os.path.join(os.path.dirname(__file__),'chinese_L-12_H-768_A-12')
CLS_DIR=os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))),'bots/cls')
cls_name=sys.argv[-1]
train_epoch=sys.argv[-2]
cls_path=os.path.join(CLS_DIR,cls_name)
data_path=os.path.join(cls_path,'data')
model_path=os.path.join(cls_path,'model')
label_list=sys.argv[-3].split(',')

## Required parameters
flags.DEFINE_string(
    "data_dir", data_path,
    "The input ner001 dir. Should contain the .tsv files (or other ner001 files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", BERT_BASE_DIR+'/bert_config.json',
    "The config json file corresponding to the pre-trained BERT model_data. "
    "This specifies the model_data architecture.")

flags.DEFINE_string("task_name", 'sim', "The name of the task to train.")

flags.DEFINE_string("vocab_file", BERT_BASE_DIR+'/vocab.txt',
                    "The vocabulary file that the BERT model_data was trained on.")

flags.DEFINE_string(
    "output_dir", model_path,
    "The output directory where the model_data checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", BERT_BASE_DIR+'/bert_model.ckpt',
    "Initial checkpoint (usually from a pre-trained BERT model_data).")
...
flags.DEFINE_bool(
    "do_predict", False,
    "Whether to run the model_data in inference mode on the test set.")
...
flags.DEFINE_integer("save_checkpoints_steps", 100,
                     "How often to save the model_data checkpoint.")

flags.DEFINE_integer("iterations_per_loop", 100,
                     "How many steps to make in each estimator ner_call.")
...
class PaddingInputExample(object):
    ...
    the entire output ner001 won't be generated.
...
class DataProcessor(object):
    """Base class for ner001 converters for sequence classification ner001 sets."""
    ...
    def get_labels(self):
        """Gets the list of labels for this ner001 set."""
        raise NotImplementedError()
...
class XnliProcessor(DataProcessor):
    """Processor for the XNLI ner001 set."""
...
class MnliProcessor(DataProcessor):
    """Processor for the MultiNLI ner001 set (GLUE version)."""
...
class MrpcProcessor(DataProcessor):
    """Processor for the MRPC ner001 set (GLUE version)."""
...
class ColaProcessor(DataProcessor):
    """Processor for the CoLA ner001 set (GLUE version)."""
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    """Creates a classification model_data."""
    ...
    # segment.
    #
    # If you want to use the token-level output, use model_data.get_sequence_output()
    # instead.
...
def main(_):
    ...
    if FLAGS.max_seq_length > bert_config.max_position_embeddings:
        raise ValueError(
            "Cannot use sequence length %d because the BERT model_data "
