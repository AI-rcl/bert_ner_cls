...
#     """
#     Run perl script to evaluate model_data
#     """
...
def print_config(config, logger):
    """
    Print configuration of the model_data
    """
...
def clean(params):
    ...
    remove saved model_data and training log
    """
...
def save_config(config, config_file):
    """
    Save configuration of the model_data
...
def load_config(config_file):
    """
    Load configuration of the model_data
    parameters are stored in json format
    """
...
def save_model(sess, model, path, logger, global_steps):
    ...
    logger.info("model_data saved")


def create_model(session, Model_class, path, config, logger):
    # create model_data, reuse parameters if exists
    model = Model_class(config)

    ckpt = tf.train.get_checkpoint_state(path)
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        logger.info("Reading model_data parameters from %s" % ckpt.model_checkpoint_path)
        #saver = tf.train.import_meta_graph('ckpt/ner.ckpt.meta')
        #saver.restore(session, tf.train.latest_checkpoint("ckpt/"))
        model.saver.restore(session, ckpt.model_checkpoint_path)
    else:
        logger.info("Created model_data with fresh parameters.")
